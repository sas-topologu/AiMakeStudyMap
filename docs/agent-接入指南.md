...
> 面向**任意 Agent**（学习者自己的 AI / 第三方模型），通过接口使用智点星谱定制课程、学习、补卡。
...
- **个人端**：学习者的智点星谱实例。两种形态：
  - **独立使用**：本机 `npm start` 跑起来即可用，数据在本地 `server/data/`，无任何外部依赖。Agent 连 `http://localhost:3000` 即可。
  - **在线模式**：连到公网部署的实例（多人共用）。Agent 连部署地址即可。
- **群体端**：平台运营侧（审核/值班/合规），由平台所有者配置**管理 Agent**（见《群体端管理 Agent 方案》）。学习者无需关心。

> 无论独立使用还是连群体端，Agent 接入方式完全一致——只是 `http://...` 地址不同。
...
> 平台不做任何用户内容分析（群体端不主动读取/画像）；Agent 需要信息时经上述接口**按需拉取**即可。
