...
制作者的提醒：此程序全部由AI制作，存在不少的问题，使用前请用AI检查一遍，如果有需要修改的地方，可以让AI修改
## 这是什么

- **个人端**：Canvas 星图 + 知识卡（推演式讲解 + 题库），闯关点亮知识；**独立使用即全功能**，不连群体端也能用。
- **断网也能学**：知识库与题库缓存在本地，离线时本地出卷、本地判分；联网后自动上报，成果标为「未认证」。
- **复习闭环**：答错的题自动进错题队列，按 10 分钟 → 1 天 → 3 天 → 7 天 → 16 天 → 35 天 间隔重复，连对 6 次即掌握。
- **数据可带走**：个人进度可导出成文件，换设备 / 换库时导入；只升不降，导回同一个库才保留认证。
- **任意 Agent 可接入**：通过 `/api/agent/*` 接口，学习者可用自己的 AI 定制课程、补缺卡、规划路线（见 `docs/agent-接入指南.md`）。
- **群体端 · 社区共创**：知识卡投稿、勘误、二创、讨论、速通榜、拓荒者纪念碑；**榜单只收已认证成果**。
